...
namespace MyNotes.Days
{
    ...
    public partial class Saturday : ContentPage
    {
            ...
            get
            {
                if (database == null)
                {
                    database = new Database(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder
                        .LocalApplicationData), "SaturdayNotifications.db3"));
                }
                return database;
            }
        }
        ...
        protected override async void OnAppearing()
        {
            ...
            var notifications = await Saturday.Database.GetNotificationAsync();
            // Уведомление напоминаний.
            Pages.NotificationsPage.CreateSystemNotifications("Saturday");
        ...
        async void AddNotificationClicked(object sender, EventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(notificationText.Text))
            {
                ...
                await Saturday.Database.SaveNotificationAsync(new Notification
                {
