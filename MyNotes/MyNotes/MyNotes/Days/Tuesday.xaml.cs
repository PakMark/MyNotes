...
namespace MyNotes.Days
{
    ...
    public partial class Tuesday : ContentPage
    {
            ...
            get
            {
                if (database == null)
                {
                    database = new Database(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder
                        .LocalApplicationData), "TuesdayNotifications.db3"));
                }
                return database;
            }
        }
        ...
        protected override async void OnAppearing()
        {
            ...
            var notifications = await Tuesday.Database.GetNotificationAsync();
            // Уведомление напоминаний.
            Pages.NotificationsPage.CreateSystemNotifications("Tuesday");
        ...
        async void AddNotificationClicked(object sender, EventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(notificationText.Text))
            {
                ...
                await Tuesday.Database.SaveNotificationAsync(new Notification
                {
